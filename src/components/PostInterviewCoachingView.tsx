...
export default function PostInterviewCoachingView({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'decision' | 'replay' | 'improve' | 'practice' | 'journey' | 'chat'>('overview');
  ...
  const handleSendChatMessage = () => {
    ...
    setTimeout(() => {
      ...
      if (query.includes('points') || query.includes('score') || query.includes('perdu')) {
        botResponse = isFR 
          ? `Vous avez principalement perdu des points sur le respect de la structure STAR (${session.resumeScore}%) et sur les questions d'adéquation technique (${session.industryScore}%). Vos conclusions manquaient d'indicateurs financiers ou de gains opérationnels chiffrés. En reformulant vos actions, veillez à toujours citer un impact tangible.`
          : `You mainly lost points on STAR Structural compliance (${session.resumeScore}%) and technical trade-off justifications (${session.industryScore}%). Your conclusions were highly descriptive but lacked hard business impact metrics. Try to frame every outcome with a quantitative KPI.`;
