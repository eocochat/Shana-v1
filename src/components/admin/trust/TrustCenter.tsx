...
type SectionTab = 'consent' | 'evaluation' | 'audit' | 'lifecycle' | 'privacy' | 'qa' | 'explainable';
...
export default function TrustCenter({ currentUser, lang = 'FR' }: TrustCenterProps) {
  ...
  // Section 7: Explainable AI State
  const [selectedExplainId, setSelectedExplainId] = useState('eval_01');
    ...
    <div className="space-y-8" id="trust-compliance-center-stage">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-stone-200" id="compliance-tab-bar">
        {[
          ...
          { id: 'explainable', label: lang === 'FR' ? '7. Explication IA & Confiance' : '7. Explainable AI & Trust', icon: Sparkles }
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      <div className="animate-fade-in" id="compliance-main-canvas">
        ...
        {/* SECTION 7: EXPLAINABLE AI & TRUST ENGINE */}
        {activeTab === 'explainable' && (
          <div className="space-y-8 text-left animate-fade-in" id="section-7-explainable-ai">
            {/* Header banner */}
            <div className="bg-white border border-stone-200 rounded-[32px] p-8 shadow-xs flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="space-y-2 max-w-2xl">
                <h3 className="font-sans font-bold text-base text-stone-900 flex items-center gap-2">
                  ...
                  <span>
                    {lang === 'FR' ? "Moteur d'Explication IA & Confiance" : "Explainable AI & Trust Engine"}
                  </span>
                </h3>
                <p className="text-xs text-[#6B7280] font-semibold leading-relaxed">
                  {lang === 'FR'
                    ? "Conformément à la réglementation européenne sur l'IA (EU AI Act), chaque score et recommandation Shana est traçable, explicable et adossé à des preuves factuelles extraites de la session."
                    : "To ensure absolute algorithmic transparency and fulfill European AI Act compliance, every candidate score and recommendation generated by Shana is mathematically traceable, verifiable, and linked back to structured transcription evidence."}
                </p>
              </div>
              <div className="px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-2xl">
                <span className="font-mono text-[10px] text-indigo-700 font-extrabold tracking-widest uppercase">
                  {lang === 'FR' ? "CONFORME EU AI ACT" : "EU AI ACT COMPLIANT"}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-4">
                <h4 className="font-mono text-[10px] uppercase tracking-wider text-[#6B7280] font-bold px-1">
                  {lang === 'FR' ? "Sélectionner un Candidat pour Inspection" : "Select Candidate for Score Verification"}
                </h4>

                <div className="space-y-3">
                  {[
                    { id: 'eval_01', name: 'Jean Candidat', role: 'SRE Specialist', score: 84, recommendation: 'HIRE', confidence: 'high' },
                    { id: 'eval_02', name: 'Clara Dubois', role: 'Senior Frontend Engineer', score: 92, recommendation: 'STRONG_HIRE', confidence: 'high' },
                    { id: 'eval_03', name: 'Arthur Pendragon', role: 'Technical Product Manager', score: 52, recommendation: 'REJECT', confidence: 'medium' },
                    { id: 'eval_04', name: 'Emma Watson', role: 'AI Research Intern', score: 78, recommendation: 'HOLD', confidence: 'low' }
                  ].map((cand) => (
                    <button
                      key={cand.id}
                      onClick={() => setSelectedExplainId(cand.id)}
                      className={`w-full p-5 rounded-3xl border text-left transition-all cursor-pointer flex justify-between items-center ${
                        selectedExplainId === cand.id
                          ? 'bg-indigo-950 border-indigo-950 text-white shadow-md'
                          : 'bg-white border-stone-200 hover:border-stone-400 text-stone-900'
                      }`}
                    >
                      <div className="space-y-1">
                        <p className="font-sans font-bold text-sm leading-none">{cand.name}</p>
                        <p className={`text-[11px] font-semibold leading-none ${selectedExplainId === cand.id ? 'text-stone-300' : 'text-stone-500'}`}>
                          {cand.role}
                        </p>
                        <div className="flex gap-2 pt-2 items-center">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-mono font-bold uppercase ${
                            cand.recommendation === 'STRONG_HIRE' || cand.recommendation === 'HIRE'
                              ? 'bg-emerald-100 text-emerald-800'
                          ...
                          <span className={`text-[9px] font-mono uppercase ${selectedExplainId === cand.id ? 'text-indigo-200' : 'text-indigo-600'}`}>
                            {cand.confidence} {lang === 'FR' ? "confiance" : "confidence"}
                          </span>
                        </div>
                      </div>

                      <div className="text-right">
                        <span className="font-sans font-bold text-2xl tracking-tight block">
                          {cand.score}%
                        </span>
                        <span className="text-[9px] font-mono text-stone-400 uppercase font-bold">Score IPS</span>
                      </div>
                    </button>
                  ))}
                </div>

                {/* Algorithmic Weight Formula Helper Card */}
                <div className="bg-stone-50 border border-stone-200 rounded-[32px] p-6 space-y-4">
                  <h4 className="font-sans font-bold text-xs text-stone-900 uppercase tracking-tight">
                    {lang === 'FR' ? "Formule d'Indice de Performance Shana" : "Shana IPS Scoring Weight Formula"}
                  </h4>
                  <p className="text-[11px] text-[#6B7280] font-semibold leading-relaxed">
                    {lang === 'FR'
                      ? "L'algorithme Shana agrège 5 dimensions selon des coefficients fixes immuables:"
                      : "The overall candidate IPS index is computed deterministically using standard immutable weights:"}
                  </p>
                  <div className="space-y-1.5 font-mono text-[10px] text-stone-700 font-semibold">
                    <div className="flex justify-between border-b border-stone-200 pb-1">
                      <span>{lang === 'FR' ? "Structure STAR" : "STAR Structure"}</span>
                      <span className="text-stone-900 font-bold">25%</span>
                    </div>
                    <div className="flex justify-between border-b border-stone-200 pb-1">
                      <span>{lang === 'FR' ? "Clarté de la diction" : "Diction Clarity"}</span>
                      <span className="text-stone-900 font-bold">25%</span>
                    </div>
                    <div className="flex justify-between border-b border-stone-200 pb-1">
                      <span>{lang === 'FR' ? "Pertinence contextuelle" : "Context Relevance"}</span>
                      <span className="text-stone-900 font-bold">20%</span>
                    </div>
                    <div className="flex justify-between border-b border-stone-200 pb-1">
                      <span>{lang === 'FR' ? "Assurance (Webcam stability)" : "Poise / Video baseline"}</span>
                      <span className="text-stone-900 font-bold">20%</span>
                    </div>
                    <div className="flex justify-between pb-1">
                      <span>{lang === 'FR' ? "Concision et Synthèse" : "Conciseness & Synthesis"}</span>
                      <span className="text-stone-900 font-bold">10%</span>
                    </div>
                  </div>
                </div>
              ...
              {/* Middle & Right Column: Score Breakdown and Verifiable Evidence */}
                  ...
                  const currentCandidate = [
                    {
                      ...
                      vocalWpm: 142,
                      fillerCount: 3,
                      gazeAlignment: "96% Perfect Alignment",
                      formula: { clarity: 85, structure: 82, confidence: 88, relevance: 86, conciseness: 75 },
                      evidenceCollected: lang === 'FR' ? [
                        "Explication détaillée post-mortem d'une panne critique de production avec mise en oeuvre immédiate d'un plan de routage alternatif.",
                        "Prise de responsabilité explicite et claire sur les délais et la communication de crise.",
                        "L'analyse acoustique indique un débit stable (142 WPM) malgré des questions volontairement déstabilisantes."
                      ] : [
                        "Detailed a complete post-mortem of a production outage with concrete steps on fallback routing.",
                        "Explicit ownership of project failures, timeline pressures, and immediate team recovery steps.",
                        "Acoustic envelope shows high poise (average 142 WPM) despite rapid-fire stress questions."
                      ],
                      evidenceMissing: lang === 'FR' ? [
                        "N'a pas évoqué de critères de délégation à long terme ou d'impact d'épanouissement sur l'équipe d'ingénierie."
                      ] : [
                        "Clear long-term team delegation metrics were not explicitly declared in scenario conclusions."
                      ],
                      actionTip: lang === 'FR' ? "Intégrez le pronom collectif 'NOUS' dans vos descriptions d'actions pour illustrer comment vous distribuez les responsabilités." : "Shift from 'I did everything' to 'I aligned the team and delegated tasks', showcasing delegation framework and collaborative impact.",
                      calibration: {
                        harshnessCheck: lang === 'FR' ? "Calibré - Score validé contre la cohorte d'ingénieurs SRE." : "Calibrated - Score validated against high-performer SRE cohorts.",
                        volatilityIndex: "Stable (9% deviation)",
                        driftStatus: lang === 'FR' ? "Aucun écart détecté" : "Zero AI Drift anomaly detected"
                      }
                    },
                    {
                      id: 'eval_02',
                      ...
                      vocalWpm: 138,
                      fillerCount: 1,
                      gazeAlignment: "99% Perfect Alignment",
                      formula: { clarity: 94, structure: 90, confidence: 95, relevance: 92, conciseness: 88 },
                      evidenceCollected: lang === 'FR' ? [
                        "Démonstration d'une compréhension fine du cycle de rendu Virtual DOM et de l'optimisation des architectures d'état complexes.",
                        "Description claire de micro-frontends scalables et de politiques d'invalidation de cache.",
                        "Adhésion parfaite au format STAR avec des métriques réelles d'impact (réduction du temps de chargement de 4.2s à 1.8s)."
                      ] : [
                        "Demonstrated deep understanding of virtual DOM reconciliation and complex state management bottlenecks.",
                        "Explained clean micro-frontend isolation patterns and cache validation under high user concurrency.",
                        "Maintained flawless structural clarity (STAR) with precise metrics on load time reduction (from 4.2s to 1.8s)."
                      ],
                      evidenceMissing: lang === 'FR' ? [
                        "N'a pas comparé de solutions alternatives comme Web Assembly pour les lourds calculs d'images (hors périmètre)."
                      ] : [
                        "Did not go deep into Web Assembly alternatives for image processing, though not strictly requested."
                      ],
                      actionTip: lang === 'FR' ? "Marquez une vraie pause de silence de 1,5 seconde juste avant de dévoiler le résultat pour lui donner du relief." : "Inject a strict 1.5-second silent pause right before presenting your final result to create professional contrast.",
                      calibration: {
                        harshnessCheck: lang === 'FR' ? "Calibré - Validé comme faisant partie des 5% de tête." : "Calibrated - Verified as genuine top tier candidate.",
                        volatilityIndex: "Very Stable (4% deviation)",
                        driftStatus: lang === 'FR' ? "Aucun écart détecté" : "Zero AI Drift anomaly detected"
                      }
                    },
                    {
                      ...
                      vocalWpm: 112,
                      fillerCount: 18,
                      gazeAlignment: "72% Eye-drifting detected",
                      formula: { clarity: 55, structure: 50, confidence: 48, relevance: 60, conciseness: 45 },
                      evidenceCollected: lang === 'FR' ? [
                        "Description satisfaisante de méthodologies de recherche utilisateur qualitative.",
                        "Bonne compréhension des enjeux commerciaux généraux."
                      ] : [
                        "Described qualitative market research steps clearly.",
                        "Adequate understanding of high-level business stakeholder goals."
                      ],
                      evidenceMissing: lang === 'FR' ? [
                        "Absence totale de résultats chiffrés ou quantifiables dans les trois exemples STAR fournis.",
                        "Densité de tics de langage élevée (tels que 'genre', 'du coup', 'euh') altérant la clarté.",
                        "Déviation significative du regard webcam et silences défensifs prolongés lors des questions de provocation."
                      ] : [
                        "Completely skipped quantitative results in all three stories.",
                        "High filler-word density (such as 'like', 'uhm') resulting in poor communication score (48%).",
                        "Exhibited significant head-pose drift and cognitive search delay during provocation questions."
                      ],
                      actionTip: lang === 'FR' ? "Concluez toujours vos récits par un chiffre précis : 'Ce qui a permis de réduire les coûts de X%...'" : "Ensure every answer finishes with a quantitative metric: 'Which directly resulted in a X% improvement in...'",
                      calibration: {
                        harshnessCheck: lang === 'FR' ? "Vérifié - Confirmé par comparaison avec la cohorte TPM. Le score reflète fidèlement la performance." : "Verified - Checked against TPMS cohort. High fidelity score confirmed.",
                        volatilityIndex: "High-stress volatility (32% deviation)",
                        driftStatus: lang === 'FR' ? "Aucun écart détecté" : "Zero AI Drift anomaly detected"
                      }
                    },
                    {
                      ...
                      vocalWpm: 155,
                      fillerCount: 11,
                      gazeAlignment: "88% Eye-alignment",
                      formula: { clarity: 80, structure: 72, confidence: 78, relevance: 82, conciseness: 80 },
                      evidenceCollected: lang === 'FR' ? [
                        "Excellente intuition mathématique et explication conceptuelle de la descente de gradient stochastique.",
                        "Indice d'adaptabilité élevé lors de questions pièges logiques inattendues."
                      ] : [
                        "Excellent mathematical intuition and conceptual explanation of gradient descent optimization.",
                        "High adaptability score during hypothetical logic-trap question."
                      ],
                      evidenceMissing: lang === 'FR' ? [
                        "Manque d'exemples chiffrés réels d'optimisations de coûts de serveurs d'apprentissage.",
                        "Le format STAR s'est effondré sur la phase finale 'Résultat' de la deuxième question."
                      ] : [
                        "Lacked quantitative metrics on model training cost optimizations.",
                        "STAR format was slightly unstructured in the 'Result' phase of the second answer."
                      ],
                      actionTip: lang === 'FR' ? "Structurez votre prochaine réponse technique en disant : 'J'ai choisi l'option A plutôt que l'option B parce que...'" : "Frame your next technical response as a trade-off choice: 'We rejected Option A in favor of Option B because...'",
                      calibration: {
                        harshnessCheck: lang === 'FR' ? "Calibré - Alerte d'instabilité déclenchée à la question #3 (chute brutale due à une hésitation cognitive)." : "Calibrated - Triggered unstable volatility warning on answer #3.",
                        volatilityIndex: "Unstable (42% deviation)",
                        driftStatus: lang === 'FR' ? "Alerte de baisse de confiance générée" : "Low confidence warning issued"
                      }
                    }
                  ].find(c => c.id === selectedExplainId) || {
                    id: 'eval_01',
                    name: 'Jean Candidat',
                    role: 'SRE Specialist',
                    score: 84,
                    ...
                    vocalWpm: 142,
                    fillerCount: 3,
                    gazeAlignment: "96% Alignment",
                    formula: { clarity: 85, structure: 82, confidence: 88, relevance: 86, conciseness: 75 },
                    evidenceCollected: [],
                    evidenceMissing: [],
                    actionTip: '',
                    calibration: { harshnessCheck: '', volatilityIndex: '', driftStatus: '' }
                  };

                  return (
                    <div className="space-y-6">
                      
                      {/* Sub-Panel: Detailed Score Justification */}
                      <div className="bg-white border border-stone-200 rounded-[32px] p-6 shadow-xs space-y-6">
                        <div className="flex justify-between items-start pb-4 border-b border-stone-100">
                          <div>
                            <h4 className="font-sans font-bold text-sm text-stone-900 uppercase">
                              {lang === 'FR' ? `Rapport de Traçabilité : ${currentCandidate.name}` : `Traceability Report: ${currentCandidate.name}`}
                            </h4>
                            <p className="text-[11px] text-[#6B7280] font-semibold">
                              {currentCandidate.role}
                            </p>
                          </div>
                          <span className={`px-2.5 py-1 rounded-full text-[10px] font-mono tracking-wider font-extrabold uppercase ${
                            currentCandidate.recommendation === 'STRONG_HIRE' || currentCandidate.recommendation === 'HIRE'
                              ? 'bg-emerald-50 border border-emerald-200 text-emerald-800'
                              : currentCandidate.recommendation === 'HOLD'
                              ? 'bg-amber-50 border border-amber-200 text-amber-800'
                              : 'bg-rose-50 border border-rose-200 text-rose-800'
                          }`}>
                            {lang === 'FR' ? "RECOMMANDATION : " : "DECISION: "}{currentCandidate.recommendation}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                          {[
                            { label: lang === 'FR' ? "Structure STAR" : "STAR Adherence", value: currentCandidate.formula.structure, weight: "25%" },
                            { label: lang === 'FR' ? "Clarté" : "Diction Clarity", value: currentCandidate.formula.clarity, weight: "25%" },
                            { label: lang === 'FR' ? "Assurance" : "Poise / Focus", value: currentCandidate.formula.confidence, weight: "20%" },
                            { label: lang === 'FR' ? "Pertinence" : "Contextual Fit", value: currentCandidate.formula.relevance, weight: "20%" },
                            { label: lang === 'FR' ? "Concision" : "Conciseness", value: currentCandidate.formula.conciseness, weight: "10%" }
                          ].map((dim, i) => (
                            <div key={i} className="bg-stone-50 border border-stone-150 p-4 rounded-2xl text-center space-y-1">
                              <span className="font-mono text-[8px] uppercase tracking-wider text-stone-400 block font-bold leading-none">{dim.label}</span>
                              <p className="font-sans font-bold text-lg text-stone-900 leading-none pt-1">{dim.value}%</p>
                              <span className="font-mono text-[8px] text-indigo-600 block font-bold leading-none">Weight {dim.weight}</span>
                            </div>
                          ))}
                        </div>

                        {/* Interactive Speech & Video Diagnostics */}
                        <div className="bg-stone-50 border border-stone-150 rounded-2xl p-4 flex flex-wrap justify-between items-center gap-4 text-xs font-semibold text-stone-700">
                          <div>
                            <span className="font-mono text-[8px] uppercase tracking-wider text-[#6B7280] block font-bold font-semibold">Speech Cadence</span>
                            <span className="font-sans font-bold text-stone-900">{currentCandidate.vocalWpm} Words/Minute</span>
                          </div>
                          <div>
                            <span className="font-mono text-[8px] uppercase tracking-wider text-[#6B7280] block font-bold font-semibold">Filler Word Count</span>
                            <span className="font-sans font-bold text-stone-900">{currentCandidate.fillerCount} fillers (low/filler metrics)</span>
                          </div>
                          <div>
                            <span className="font-mono text-[8px] uppercase tracking-wider text-[#6B7280] block font-bold font-semibold">Focus Vectors</span>
                            <span className="font-sans font-bold text-stone-900">{currentCandidate.gazeAlignment}</span>
                          </div>
                        </div>

                        {/* Evidence Collected */}
                        <div className="space-y-3">
                          <h5 className="font-sans font-bold text-xs text-stone-900 flex items-center gap-1.5 uppercase">
                            <CheckCircle className="w-4 h-4 text-emerald-600 shrink-0" />
                            <span>{lang === 'FR' ? "Preuves Factuelles Identifiées (Liaison Sémantique)" : "Verifiable Transcription Evidence"}</span>
                          </h5>
                          <div className="bg-stone-50 border border-stone-150 p-5 rounded-2xl space-y-3">
                            {currentCandidate.evidenceCollected.map((ev, idx) => (
                              <p key={idx} className="text-xs text-stone-700 italic leading-relaxed pl-3 border-l-2 border-emerald-500 font-medium">
                                "{ev}"
                              </p>
                            ))}
                          </div>
                        ...
                        {/* Evidence Missing */}
                        {currentCandidate.evidenceMissing.length > 0 && (
                          <div className="space-y-3">
                            <h5 className="font-sans font-bold text-xs text-stone-900 flex items-center gap-1.5 uppercase">
                              <XCircle className="w-4 h-4 text-rose-600 shrink-0" />
                              <span>{lang === 'FR' ? "Lacunes de Preuves / Pénalités d'Indices" : "Unsubstantiated Criteria / Penalty Flags"}</span>
                            </h5>
                            <div className="bg-stone-50 border border-stone-150 p-5 rounded-2xl space-y-3">
                              {currentCandidate.evidenceMissing.map((ev, idx) => (
                                <p key={idx} className="text-xs text-stone-600 leading-relaxed pl-3 border-l-2 border-rose-500 font-medium">
                                  {ev}
                                </p>
                              ))}
                            </div>
                        ...
                        {/* Improvement Coaching Track */}
                        <div className="space-y-3">
                          <h5 className="font-sans font-bold text-xs text-stone-900 flex items-center gap-1.5 uppercase">
                            <Activity className="w-4 h-4 text-indigo-600 shrink-0" />
                            <span>{lang === 'FR' ? "Plan d'Amélioration Recommandé" : "Algorithmic Coaching Track"}</span>
                          </h5>
                          <div className="bg-indigo-50 border border-indigo-150 p-4 rounded-2xl">
                            <p className="text-xs text-indigo-950 leading-relaxed font-semibold">
                              {currentCandidate.actionTip}
                            </p>
                          </div>
                        </div>
                      ...
                      {/* Anti-Bias Calibration & Model Drift Audit */}
                      <div className="bg-stone-950 text-stone-100 border border-stone-800 rounded-[32px] p-6 space-y-6 shadow-xl">
                        <div className="border-b border-stone-800 pb-4">
                          <h4 className="font-sans font-bold text-sm text-white uppercase tracking-tight">
                            {lang === 'FR' ? "Sécurité Algorithmique & Calibration Anti-Biais" : "Algorithmic Integrity & Drift Calibration"}
                          </h4>
                          <p className="text-[11px] text-stone-400 font-semibold">
                            {lang === 'FR' ? "Audits automatiques pour prévenir l'indulgence ou la sévérité excessive de l'IA." : "Automatic sanity loops protecting against artificial score compression or model hallucination."}
                          </p>
                        </div>

                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-semibold text-xs text-stone-300">
                          <div className="space-y-1">
                            <span className="font-mono text-[9px] uppercase tracking-wider text-stone-500 block font-bold font-semibold">Harshness Check</span>
                            <p className="font-sans font-bold text-white text-[13px]">{currentCandidate.calibration.harshnessCheck}</p>
                          </div>
                          <div className="space-y-1">
                            <span className="font-mono text-[9px] uppercase tracking-wider text-stone-500 block font-bold font-semibold">Volatility Index</span>
                            <p className="font-sans font-bold text-white text-[13px]">{currentCandidate.calibration.volatilityIndex}</p>
                          </div>
                          <div className="space-y-1">
                            <span className="font-mono text-[9px] uppercase tracking-wider text-stone-500 block font-bold font-semibold">Drift Metric</span>
                            <p className="font-sans font-bold text-white text-[13px]">{currentCandidate.calibration.driftStatus}</p>
                          </div>
                        </div>
                      </div>
